...
export default function NotificationBell({
  ...
  return (
    <div className="relative" ref={containerRef}>
      {!hideTrigger ? (
        <button
          className={`relative rounded-full border border-slate-200 bg-white p-3 text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 ${buttonClassName}`.trim()}
          onClick={togglePanel}
          type="button"
        >
      ...
      {isOpen ? (
        <div
          className={`absolute right-0 z-30 mt-3 w-96 max-w-[calc(100vw-2rem)] rounded-3xl border border-slate-200 bg-white p-4 shadow-xl ${panelClassName}`.trim()}
        >
